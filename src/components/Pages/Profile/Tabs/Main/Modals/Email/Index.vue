...
<template>
    ...
    >
        <div class="flex w-full flex-col items-center rounded-[20px] bg-[#1B1C1E] px-[25px] pb-[30px] pt-[23px]">
            ...
            <PagesProfileTabsMainModalsEmailContentProtect @next="step = 2" v-if="step === 1" />
            <PagesProfileTabsMainModalsEmailContentAdd @next="step = 3" v-if="step === 2" />
            <PagesProfileTabsMainModalsEmailContentConfirm v-if="step === 3" />
        </div>
    </VueFinalModal>
</template>
