...
<template>
    <div class="flex h-[70px]">
        <div class="relative z-10 flex h-full w-full items-center justify-between rounded-[55px] border-[1px] border-[#232426] bg-[#1A1B1D] px-[22px]">
            <LayoutMobileNavBarItem :isActive="isShowSidebar" @click="emit('toggle-sidebar')" :link="{ title: 'Меню', img: iconMenu }" />

            <div v-click-outside="() => (activeLink === 'bonuses' ? (activeLink = null) : '')">
                <LayoutMobileNavBarItem
                ...
                />
                <LayoutMobileNavBarMenuBonus v-if="activeLink === 'bonuses'" class="absolute bottom-[calc(100%+8px)] left-0 right-0 z-0" />
            </div>
            ...
            <div v-click-outside="() => (activeLink === 'games' ? (activeLink = null) : '')">
                ...
                />
                <LayoutMobileNavBarMenuGames v-if="activeLink === 'games'" class="absolute bottom-[calc(100%+8px)] left-0 right-0 z-0" />
            </div>
            ...
            <div v-click-outside="() => (activeLink === 'bets' ? (activeLink = null) : '')">
                ...
                />
                <LayoutMobileNavBarMenuBets v-if="activeLink === 'bets'" class="absolute bottom-[calc(100%+8px)] left-0 right-0 z-0" />
            </div>
            ...
            <div v-if="isAuth" v-click-outside="() => (isShowModalProfile = false)">
                <button @click="isShowModalProfile = !isShowModalProfile" class="relative flex h-full flex-col items-center justify-between py-[7px]">
                    ...
                    >
                        <div
                            class="absolute -bottom-[6px] flex h-[11px] items-center justify-center rounded-[6px] bg-[#84FD4B] px-2 text-[10px] text-[#151618]"
                        >
                            10
                        </div>
                    </div>
                ...
                <PopupsProfileIndex
                    ...
                    @logout="handleLogout"
                    @modal-notifications="$emit('modal-notifications')"
                />
            </div>
        </div>
